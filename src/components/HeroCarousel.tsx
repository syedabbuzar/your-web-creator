...
const HeroCarousel = () => {
  ...
  return (
    <section className="relative w-full h-[200px] sm:h-[280px] md:h-[380px] lg:h-[450px] xl:h-[520px] overflow-hidden">
      ...
      <button onClick={(e) => { e.preventDefault(); next(); }} className="absolute right-2 sm:right-3 md:right-4 top-1/2 -translate-y-1/2 z-30 w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center transition-colors" aria-label="Next slide">
      ...
      <div className="absolute bottom-1.5 sm:bottom-2 left-1/2 -translate-x-1/2 z-30 flex gap-1.5 sm:gap-2">
        {events.map((_: any, index: number) => (
          <button key={index} onClick={(e) => { e.preventDefault(); setCurrent(index); }} className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full transition-all ${index === current ? "bg-white w-4 sm:w-6" : "bg-white/50"}`} aria-label={`Go to slide ${index + 1}`} />
        ))}
      </div>
    </section>
  );
