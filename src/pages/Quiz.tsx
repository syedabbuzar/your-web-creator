...
// ============ TYPES ============
type QuizView = "landing" | "register" | "login" | "quiz" | "result" | "admin-panel" | "change-class";
...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    const savedUser = JSON.parse(localStorage.getItem("scholar_quiz_user") || "null");
  ...
  const loadAdminQuestions = async () => {
    try {
      ...
      setAdminQs(Array.isArray(data.questions) ? data.questions : Array.isArray(data) ? data : []);
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Failed to load questions");
    }
  };
  ...
  const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      localStorage.setItem("scholar_quiz_user", JSON.stringify(u));
  ...
  const handleChangeClass = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      await axiosInstance.post("/auth/change-class", {
        email: form.email.trim(),
        password: form.password.trim(),
        newClass: parseInt(form.newClass),
      });
  ...
  const handleAdminSave = async (data: AdminQuestionFormData) => {
    ...
    try {
      if (editingQ) {
        await axiosInstance.put(`/questions/${editingQ._id}`, {
      ...
      await loadAdminQuestions();
      const countsRes = await axiosInstance.get("/questions/counts").catch(() => ({ data: { counts: {} } }));
  ...
  const AdminQuestionForm = ({ initial, onSave, onCancel }: { initial?: QuizQuestion | null; onSave: (data: AdminQuestionFormData) => void; onCancel: () => void }) => {
    const [f, setF] = useState({
      question: initial?.question || "",
      class: initial?.class?.toString() || "1",
      options: initial?.options?.map((o) => o.text) || ["", "", ""],
      correctIdx: initial ? initial.options.findIndex((o) => o.id === initial.correctOptionId) : 0,
    });
    const submit = (e: FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      if (!f.question.trim() || f.options.some((o) => !o.trim())) {
    ...
    return (
      <form onSubmit={submit} className="space-y-4">
        <div className="space-y-2">
          ...
          <Textarea value={f.question} onChange={(e) => setF({ ...f, question: e.target.value })} placeholder="Enter question..." rows={3} required />
        </div>
        <ClassSelect value={f.class} onChange={(v) => setF({ ...f, class: v })} label="Class *" />
        <div className="space-y-3">
          ...
          {f.options.map((opt, idx) => (
            <div key={idx} className="flex items-center gap-3 p-2 rounded hover:bg-secondary/30">
              ...
              <Input value={opt} onChange={(e) => { const o = [...f.options]; o[idx] = e.target.value; setF({ ...f, options: o }); }} placeholder={`Option ${String.fromCharCode(97 + idx)}`} required />
  ...
  const renderAdmin = () => {
    if (!isAdmin) { setView("landing"); return null; }
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
        ...
        <Tabs defaultValue="questions" className="w-full">
              ...
              <CardContent className="pt-4">
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1 relative">
                    ...
                    <Input placeholder="Search questions..." value={adminSearch} onChange={(e) => setAdminSearch(e.target.value)} className="pl-9" />
                  </div>
                  <ClassSelect value={adminFilter} onChange={setAdminFilter} showAll />
                  <Dialog>
          ...
          <TabsContent value="students" className="space-y-4 mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Quiz Attempted</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-green-600">{stats?.attemptedQuiz || studentsList.filter(s => s.quizAttempted).length}</div></CardContent></Card>
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Not Attempted</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-yellow-600">{stats?.notAttempted || studentsList.filter(s => !s.quizAttempted).length}</div></CardContent></Card>
            </div>
            <Card>
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                  ...
                  <ClassSelect value={studentClassFilter} onChange={setStudentClassFilter} showAll />
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {filteredStudents.map((student) => (
                        <tr key={student._id} className="hover:bg-secondary/20">
                          <td className="py-3"><div><p className="font-medium">{student.name}</p><p className="text-xs text-muted-foreground">{student.email}</p></div></td>
