...
const Index = () => {
  ...
  return (
    <Layout>
      <SEOHead 
        title="Best School in Nanded | Nursery to 12th Grade"
        description="Scholar Educational Campus Nanded - Top ranked school offering quality education from Nursery to 12th Grade. Smart classrooms, experienced faculty, scholarship programs. Admissions open. Hyderabad Road Wajegaon, Nanded."
        keywords="best school in nanded, top school nanded, scholar educational campus, school admission nanded, CBSE school nanded, nanded school, smart class school nanded, nursery school nanded, 12th school nanded, scholarship exam nanded, best english medium school nanded"
        path="/"
      />
      <FloatingActions />
      <HeroCarousel />
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-sm sm:text-base md:text-lg lg:text-xl text-muted-foreground uppercase tracking-widest mb-3 sm:mb-4 md:mb-6 animate-fade-in" style={{ animationDelay: "0.1s" }}>
              Welcome to Scholar Educational Campus Nursery To 12th Grade
            </h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 animate-fade-in-up" style={{ animationDelay: "0.7s" }}>
              <Link to="/about">
                <Button className="btn-hover bg-primary text-primary-foreground text-sm sm:text-base md:text-lg px-6 sm:px-8 py-4 sm:py-5 md:py-6 rounded-full font-semibold group">
                  EXPLORE MORE <ArrowRight className="ml-2 w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
                </Button>
              </Link>
              <Link to="/quiz">
                <Button variant="outline" className="btn-hover border-primary text-primary hover:bg-primary hover:text-primary-foreground text-sm sm:text-base md:text-lg px-6 sm:px-8 py-4 sm:py-5 md:py-6 rounded-full font-semibold">🎓 APPLY SCHOLARSHIP</Button>
              </Link>
              <Link to="/admission">
                <Button className="btn-hover bg-accent text-accent-foreground text-sm sm:text-base md:text-lg px-6 sm:px-8 py-4 sm:py-5 md:py-6 rounded-full font-semibold">📝 ADMISSION</Button>
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {events.map((event, index) => {
              ...
              return (
                <div key={eventId || index} className="animate-fade-in-up opacity-0" style={{ animationDelay: `${0.2 + index * 0.15}s`, animationFillMode: "forwards" }}>
                  <EventCard id={eventId} title={event.title} description={event.description} date={event.date} image={event.image} images={Array.isArray(event.images) ? event.images : undefined} className="hover-scale" />
                </div>
              );
            })}
          </div>
          <div className="text-center mt-8 sm:mt-10 md:mt-12">
            <Link to="/events">
              <Button variant="outline" className="btn-hover border-primary text-primary hover:bg-primary hover:text-primary-foreground px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold text-sm sm:text-base">View All Events</Button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 sm:py-14 md:py-20 bg-primary text-primary-foreground">
        <div className="container mx-auto px-4 text-center">
          ...
          <Link to="/contact" className="animate-fade-in-up inline-block" style={{ animationDelay: "0.2s" }}>
            <Button className="bg-primary-foreground text-primary hover:bg-primary-foreground/90 text-sm sm:text-base md:text-lg px-6 sm:px-8 py-4 sm:py-5 md:py-6 rounded-full font-semibold btn-hover">Get In Touch</Button>
          </Link>
        </div>
      </section>
    </Layout>
